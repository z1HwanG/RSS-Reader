...
**项目状态**：`0.7.1`，早期开发阶段，功能与持久化结构（`state.json` 的 `schema_version`）仍可能变动。
...
| 文件 | 说明 |
|------|------|
| `RSSReader_0.7.1_x64-setup.exe` | NSIS 安装程序（推荐） |
| `RSSReader_0.7.1_x64_en-US.msi` | MSI 安装包 |
| `RSSReader_0.7.1_x64_portable.exe` | 免安装单文件，系统需已有 WebView2 |
